...
| MOBILE_CALLBACK | OWNER_ACTION_REQUIRED | `com.loop.app.loop_mobile://login-callback` confirmed correct in source (`auth_screen.dart`, matching the Android manifest's intent-filter exactly) — but real owner testing found it isn't yet in the Supabase project's Auth > URL Configuration > Redirect URLs allow-list, so both Google OAuth and email confirmation currently fall back to the website instead of handing off to the app. Platform config, not reachable via any tool this session has (confirmed absent from every queryable `auth.*` table). See docs/KNOWN_ISSUES.md for the exact 2-step fix. |
...
| ANDROID_INTENT_FILTER | PASS | Directly verified live via `adb shell am start -a android.intent.action.VIEW -d "com.loop.app.loop_mobile://login-callback"` with LOOP backgrounded first — Android correctly brought LOOP's task to the foreground (confirmed by screenshot), not Chrome. Rules out the manifest/intent-filter as a cause of the reported redirect bug; the gap is isolated to the Supabase Redirect URLs allow-list (see MOBILE_CALLBACK). |
